...
impl LayerId {
    ...
    /// Layers the figure is allowed to occupy.
    pub const fn is_stickman_layer(self) -> bool {
        matches!(self, Self::Middle | Self::Foreground)
    }

    /// Clamp to a valid figure layer (background → middle).
...
/// Clear the previous figure and restore layer-0 pixels under it.
///
...
pub fn restore_background_under_actor<D>(
    display: &mut D,
    actor: &Actor,
    pose: &PoseScratch,
    prev_rect: Rectangle,
    ...
    D: DrawTarget<Color = Rgb565>,
{
    if let Some(img) = background {
        ...
        render::draw_actor_colored(display, actor, pose, Rgb565::BLACK)?;
        let (x0, x1) = render::pose_floor_dirty_x(pose);
        render::draw_floor_span(display, x0, x1)?;
    }
    Ok(())
}

/// Draw the figure onto its depth layer (1 or 2). Does not touch layer 0.
pub fn draw_actor_layer<D>(
    display: &mut D,
    actor: &Actor,
    pose: &PoseScratch,
) -> Result<(), D::Error>
where
    D: DrawTarget<Color = Rgb565>,
{
    let layer = actor.layer.clamp_stickman();
    match layer {
        LayerId::Middle | LayerId::Foreground => render::draw_actor(display, actor, pose),
        LayerId::Background => Ok(()),
    }
}

/// Full back-to-front compose (background + figure).
pub fn compose<D>(
    display: &mut D,
    actor: &Actor,
    pose: &PoseScratch,
    ...
    D: DrawTarget<Color = Rgb565>,
{
    ...
    draw_actor_layer(display, actor, pose)
}
